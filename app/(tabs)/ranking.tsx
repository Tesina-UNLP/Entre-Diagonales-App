    ...
    <ThemedBackground style={styles.container}>
      <FlatList
        ...
        renderItem={({ item, index }) => (
          ...
          <FadeInView delay={600 + Math.min(index * 40, 200)} key={`ranking-${activeTab}-${index}`}>
            <View style={styles.rowCard}>
              <ThemedText type="subtitle" style={styles.position}>{item.position}</ThemedText>

              <Image source={{ uri: item.character }} style={styles.rowAvatar} />

              <View style={{ flex: 1, gap: 4 }}>
                <ThemedText type="subtitle">
                  {item?.display_name?.slice(0, 40) ?? item.username?.slice(0, 40)}
                </ThemedText>
